...
public class Upg_Gimbal
{
	...
	public int UpgradeStallTimeoutMs { get; set; } = 8000;
	...
	public event Action<bool, ConstEnum.RecvPacket_Gim, object> DeviceFrameInfo;
	...
	private void OnAck(uint cmd, object data, object data2)
	{
		...
		case AR_COMMAND.AR_CMD_SENDFILE_END:
		{
			...
			string text = ((GD.Inst.CurrLang == 1) ? "提示" : "Prompt");
			string text2 = ((GD.Inst.CurrLang == 1) ? "文件上传成功" : "File upload successfully");
	...
	{
		ConstEnum.RecvPacket_Gim arg2 = new ConstEnum.RecvPacket_Gim();
		...
		switch ((ConstEnum.GIM_CMD)_currCMD)
		{
		case ConstEnum.GIM_CMD.CalibGyro:
			if (arg2.index != 7 && _isCalibGryo)
			{
				await Task.Delay(_sendCalibDelay);
				_calibPlusing++;
				BuildSendPacket_Gimbal(ConstEnum.GIM_CMD.CalibGyro);
			...
			else
			{
				...
				_isCalibGryo = false;
				BuildSendPacket_Gimbal(ConstEnum.GIM_CMD.StartGim);
			...
			break;
		case ConstEnum.GIM_CMD.StartGim:
			if (arg2.index != 4)
			{
				await Task.Delay(_sendCommDelay);
			}
			break;
		case ConstEnum.GIM_CMD.StopGim:
			if (arg2.index != 5)
			{
				await Task.Delay(_sendCommDelay);
			}
			break;
		case ConstEnum.GIM_CMD.WriteParam:
			if (arg2.index != 14)
			{
				await Task.Delay(_sendCommDelay);
			}
			break;
		case ConstEnum.GIM_CMD.SetGMMode:
			if (arg2.index != 1)
			{
				await Task.Delay(_sendCommDelay);
			}
			break;
		case ConstEnum.GIM_CMD.SetSensNum:
			if (arg2.index != 1)
			{
				await Task.Delay(_sendCommDelay);
			}
			break;
		case ConstEnum.GIM_CMD.SetRollNum:
			if (arg2.index != 23)
			{
				await Task.Delay(_sendCommDelay);
			}
			break;
		case ConstEnum.GIM_CMD.SetPitchNum:
			if (arg2.index != 24)
			{
				await Task.Delay(_sendCommDelay);
			}
			break;
		case ConstEnum.GIM_CMD.SetYawNum:
			if (arg2.index != 25)
			{
				await Task.Delay(_sendCommDelay);
			}
	...
	private bool ParseCommData(byte[] recBuffer, out ConstEnum.RecvPacket_Gim packet)
	{
		bool result = false;
		packet = new ConstEnum.RecvPacket_Gim();
		try
		{
	...
	private void ResUpgradeInfo_Asce(byte[] arg1, object arg2, object arg3)
	{
		try
		{
			...
			case AR_COMMAND.AR_COMMAND_FIND_DEVICE:
			{
				ResDeviceInfo resDeviceInfo = ParseDeviceInfo(arg1);
				break;
			}
			case AR_COMMAND.AR_COMMAND_REBOOT:
			{
	...
	private void StartTimeout(uint ms, string msg)
	{
		_ctsTimeout?.Cancel();
		_ctsTimeout = new CancellationTokenSource();
		CancellationToken token = _ctsTimeout.Token;
	...
	private void SendFirmwareChunk(int offset, int size)
	{
		...
		if (num >= 100)
		{
			...
			HappenEventArgs e2 = new HappenEventArgs
			{
				...
				eventType = EventType.ackTrigger,
				processVal = 1f,
				processDesc = ((GD.Inst.CurrLang == 1) ? "升级完成" : "Upgrade is complete")
			};
	...
	public async Task BuildSendPacket_Gimbal(ConstEnum.GIM_CMD cmd, int delayTime)
	{
		try
		{
			await Task.Delay(delayTime);
			_currCMD = (byte)cmd;
			if (cmd == ConstEnum.GIM_CMD.CalibGyro)
			{
			}
			List<byte> frameData = new List<byte>
			{
				126,
				22,
				(byte)(cmd & (ConstEnum.GIM_CMD)0xFF)
			};
			short rollcm = ClampToInt16(0f);
			short pitchcm = ClampToInt16(0f);
			short yawcm = ClampToInt16(0f);
			frameData.Add((byte)((rollcm >> 8) & 0xFF));
			frameData.Add((byte)(rollcm & 0xFF));
			frameData.Add((byte)((pitchcm >> 8) & 0xFF));
			frameData.Add((byte)(pitchcm & 0xFF));
			frameData.Add((byte)((yawcm >> 8) & 0xFF));
			frameData.Add((byte)(yawcm & 0xFF));
			frameData.Add(90);
			frameData.Add(165);
			_usb.SendMsg(frameData.ToArray());
		}
		catch (Exception ex)
		{
			Exception ex2 = ex;
			WriteLog.WriteLogFileToUI("BuildSendPacket_Gimbal error,desc=" + ex2.Message, Color.DarkRed);
		}
	}

	public int BuildSendPacket_Gimbal(ConstEnum.GIM_CMD cmd, float rollcmd = 0f, float pitchcmd = 0f, float yawcmd = 0f)
	{
		try
		{
			_currCMD = (byte)cmd;
			...
			{
				_isCalibGryo = true;
				StartTimeout(3000u, "CalibGyro");
			}
			List<byte> list = new List<byte>();
			list.Add(126);
			list.Add(22);
			list.Add((byte)(cmd & (ConstEnum.GIM_CMD)0xFF));
