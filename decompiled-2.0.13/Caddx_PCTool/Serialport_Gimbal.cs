...
public class Serialport_Gimbal : IDisposable
{
	...
	private readonly byte[] _frameBuf = new byte[256];
	...
	private void SendQueueLoop(CancellationToken cts)
	{
		...
		{
			WriteLog.WriteLogFileToUI("发送线程异常,desc=" + ex.Message, Color.Red);
		}
	}
	...
	private void SerialportSendMethod()
	{
		...
		{
			WriteLog.WriteLogFileToUI("串口发送异常,desc=" + ex.Message, Color.Red);
		}
	}
	...
	private void ParseBuffer()
	{
		...
		while (true)
		{
			int num = _recvBuffer.PeekHeader(_headerBuf, 4);
			if (num < 0)
			{
				break;
			}
			byte b = _headerBuf[0];
			byte b2 = _headerBuf[1];
			byte b3 = _headerBuf[2];
			byte b4 = _headerBuf[3];
			int num2 = 0;
			if (b == 234 && b2 == 85)
			{
				num2 = 17;
			...
			else if (b == 171 && b2 == byte.MaxValue)
			{
				if (IsUpgrading)
				{
					if (num >= 125)
					{
						_recvBuffer.Skip(125);
						continue;
					}
					break;
				}
	...
	private void ProcessFrame(byte[] frame, uint asce_cmd)
	{
		...
		{
			if (b == 171 && b2 == byte.MaxValue)
			{
				ResFrameInfo?.Invoke(frame, null, null);
			}
			else if (b == 79 && b2 == 84 && b3 == 82 && b4 == 65)
			{
	...
	public bool HandlePacket(byte[] recBuffer, ConstEnum.RecvPacket_Gim packet)
	{
		bool result = false;
		if (recBuffer.Length == 118 && recBuffer[0] == 171 && recBuffer[1] == byte.MaxValue)
		{
			result = ParseCommData(recBuffer, new ConstEnum.RecvPacket_Gim());
		}
	...
	private bool ParseCommData(byte[] recBuffer, ConstEnum.RecvPacket_Gim packet)
	{
		bool result = false;
		try
		{
	...
	public int BuildSendPacket(ConstEnum.GIM_CMD cmd, float roll = 0f, float picth = 0f, float yaw = 0f)
	{
